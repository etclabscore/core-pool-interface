...
export default {
  props: {
    ...
    clip: { // shorten inner url to (0x[clip][separator][clip]) characters. 0 = dont clip.
      type: Number,
      default() {
        return 0
      }
  ...
  methods: {
    formatUrl() {
      ...
      let type = this.config.explorer.type
      let network = this.config.network
      let symbol = this.config.symbol.toLowerCase()
      let append = '/'
      ...
      switch(this.linkType) {
        case 'block':
          ...
          } else { // etherscan, etherchain or expedition, spectrum
            append = append + 'block/'
          }
          break
        case 'account':
          ...
          } else { // etherscan, blockscout, expedition, spectrum
        ...
        case 'token':
          ...
          } else { // etherscan, etherchain, expedition, spectrum
            append = append + 'token/'
          }
          break
        case 'tx':
